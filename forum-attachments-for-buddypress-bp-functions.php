...
/**
 * Attempt the initial table installation
 *
 * @package Forum Attachments for BuddyPress
 */
...
add_action( 'admin_init', 'bp_forum_attachments_check_installed', 50 );
...
function bp_forum_attachments_add_js() {
	...
	if ( bp_is_group_forum() && is_user_logged_in() ) {
		...
		foreach ($bb_attachments_cache[$post_id] as $attachment) {
			...
			if ($attachment->status==0 || empty($filter) || $showerror) {
				...
				if ( $attachment->status<2 ) {
					$output .= ", " . bb_number_format_i18n( $attachment->downloads ) . " " . __( 'downloads', 'fa-4-buddypress' ) ;
				}
				$output.=")</span> ";
...
function bb_number_format_i18n( $number, $decimals = null ) {
    global $bb_locale;
     // let the user override the precision only
      $decimals = ( is_null( $decimals ) ) ? isset( $bb_locale->number_format['decimals'] ) : intval( $decimals );
...
function bb_attachments_process_post($group_id, $post, $display=0) {
	...
	if ( is_object( $post ) && isset( $post->topic_last_post_id ) ) {
...
function bb_attachments_upload_form( $post_id = 0 ) {
	...
	if ( $post_id ) {
		echo '<form class="bb_attachments_upload_form" enctype="multipart/form-data" method="post" action="' . esc_attr( add_query_arg( 'bb_attachments', $post_id, remove_query_arg( array( 'bb_attachments', 'bbat', 'bbat_delete' ) ) ) ) . '">';
	} else {
		echo '<div class="bp-forum-attachments-reply"><input  type="hidden" name="bb_attachments" value="0" />';
	}
	...
	if ( $post_id ) {
		echo '<a style="margin-right:12em;" href="' . get_post_link( $post_id ) . '">' . __( "&laquo; return to post", "fa-4-buddypress" ) . '</a>';
	}
...
function bb_attachments_download( $filenum = 0 ) {
	...
	if ( $filenum == 0 && isset( $_GET['bbat'] ) ) {
		$filenum = intval( $_GET['bbat'] );
	}

	if ( $filenum > 0 && $bb_attachments['role']['download'] == "read" ) {
		$file = $wpdb->get_results("SELECT * FROM ".$bb_attachments['db']." WHERE id = $filenum AND status = 0 LIMIT 1");
...
if ($filenum>0 && $bb_attachments['role']['inline']=="read" ) {
	$file=$wpdb->get_results("SELECT * FROM ".$bb_attachments['db']." WHERE id = $filenum AND status = 0 LIMIT 1");
	if (isset($file[0]) && $file[0]->id) {
		$file=$file[0]; $file->filename=stripslashes($file->filename);
		$path=$bb_attachments['path'].floor($file->id/1000)."/";
...
/**
 * @todo Make sure only the correct folks can delete
 */
