...
$bb_attachments['icons']['url']=get_option('home').trim(str_replace(array(trim(BBPATH,"/\\"),"\\"),array("","/"),dirname(__FILE__)),' /\\').'/icons/'; 
...
register_activation_hook(str_replace(array(str_replace("/","\\",BB_PLUGIN_DIR),str_replace("/","\\",BB_CORE_PLUGIN_DIR)),array("user#","core#"),__FILE__), 'bb_attachments_install');
		...
		foreach ($bb_attachments_cache[$post_id] as $attachment) { 
			...
			if ($attachment->status==0 || empty($filter) || $showerror) {
				...
				if ($attachment->status>0 && (empty($filter) || $showerror)) {					
					...
					//	bb_attachments_inline($attachment->id);
...
//if (!is_bb_feed()) {
	global $bb_post, $bb_attachments, $topic_template;
	$post_id = $topic_template->post->post_id;
	$text ="<div class='bb_attachments'>".bb_attachments_post_attachments($post_id)."</div>";
//}
echo $text;
}
